...
class Organizador:
    def __init__(self, largura_l, altura_l, num_linhas):
        ...
        self.espacos_livres = [[] for _ in range(num_linhas)] # espaços livres
    ...
    def calcular_ocupacao(self):
        ...
        for i in range(self.num_l):
            # A área ocupada por cada linha é a largura usada vezes a altura da linha
            area_ocupada += self.linhas[i] * self.altura_l

        return (area_ocupada / area_total) * 100  # percentual
...
class InterfaceTk:
    ...
    def remover_celula(self):
        ...
        if celula:
            ...
            # Ordena por posição para facilitar futuras fusões (opcional)
